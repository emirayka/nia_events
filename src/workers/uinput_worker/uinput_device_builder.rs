...
impl UInputDeviceBuilder {
    pub fn default() -> Result<UInputDeviceBuilder, Error> {
        let result = uinput::default()
            .map(|device_builder| UInputDeviceBuilder { device_builder })
    ...
    pub fn from(device_builder: uinput::device::Builder) -> UInputDeviceBuilder {
        UInputDeviceBuilder { device_builder }
    }
    ...
    pub fn enable_keyboard(self) -> Result<UInputDeviceBuilder, Error> {
        let result = self
            ...
            .map_err(|_| {
                Error::uinput_configuration_error(
                    "Cannot enable keyboard events for uinput device.",
                )
            })?;
    ...
    pub fn enable_mouse(self) -> Result<UInputDeviceBuilder, Error> {
        ...
        let result = self
            ...
            .map_err(|_| {
                Error::uinput_configuration_error("Cannot enable left button of uinput device.")
            })?
            .event(Controller(Mouse(Right)))
            .map_err(|_| {
                Error::uinput_configuration_error("Cannot enable right button of uinput device.")
            })?
            .event(Controller(Mouse(Middle)))
            .map_err(|_| {
                Error::uinput_configuration_error("Cannot enable middle button of uinput device.")
            })?
            .event(Controller(Mouse(Side)))
            .map_err(|_| {
                Error::uinput_configuration_error("Cannot enable side button of uinput device.")
            })?
            .event(Controller(Mouse(Extra)))
            .map_err(|_| {
                Error::uinput_configuration_error("Cannot enable extra button of uinput device.")
            })?
            .event(Controller(Mouse(Forward)))
            .map_err(|_| {
                Error::uinput_configuration_error("Cannot enable forward button of uinput device.")
            })?
            .event(Controller(Mouse(Back)))
            .map_err(|_| {
                Error::uinput_configuration_error("Cannot enable back button of uinput device.")
            })?
            .event(Controller(Mouse(Task)))
            .map_err(|_| {
                Error::uinput_configuration_error("Cannot enable task button of uinput device.")
            })?;
